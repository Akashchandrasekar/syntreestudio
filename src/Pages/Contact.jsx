...
const ContactUs = () => {
  ...
  const handleGetInTouch = () => {
    if (messageDivRef.current) {
      ...
      messageDivRef.current.classList.add("animate-focus");
      shopNameRef.current.focus();

      setTimeout(() => {
        messageDivRef.current.classList.remove("animate-focus");
      }, 500);
    }
  };
  ...
  return (
    <div
      className="relative h-fit text-white font-raleway"
      style={{
  ...
  Contact <span className="text-4xl text-pink-500">Us </span> <Mail className="text-pink-500 w-10 h-10" />
</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {/* Contact Info Section */}
          <div className="relative max-w-xl text-right p-6 rounded-lg shadow-lg">
            <h3 className="text-white text-4xl py-4 font-semibold">
              Contact <span className="text-pink-500">Info</span>
            </h3>
            <p className="text-white text-2xl py-2 flex items-center justify-end">
              <MapPin className="mr-2 text-pink-500" />
              North Street, Nallur Kumbakonam 614208
            </p>
            <p className="text-white text-2xl py-2 flex items-center justify-end">
              <Phone className="mr-2 text-pink-500" />
              9092035313 | 9344794518
            </p>
            <p className="text-white text-2xl py-2 flex items-center justify-end">
              <Mail className="mr-2 text-pink-500" />
              syntreestudio@gmail.com
            </p>
            <button
              onClick={handleGetInTouch}
              className="bg-blue-500 text-white px-6 py-3 rounded cursor-pointer transition transform hover:bg-blue-600 hover:scale-105"
            >
              Get in Touch &rarr;
            </button>
          </div>
          ...
          >
            <div className="bg-black bg-opacity-70 p-6 rounded-lg">
              ...
              <button
                onClick={handleSend}
                className="bg-pink-500 text-white p-3 w-full cursor-pointer rounded-md hover:bg-pink-600 transition duration-300"
              >
                Send
              </button>
            </div>
